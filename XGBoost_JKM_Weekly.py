...
df = pd.read_csv('JKM_Weekly_Price.csv')
...
def learning_rate_schedule(round_number):
    if round_number < 300100:
        return 0.0001
    elif round_number < 100000:
        return 0.00005
    else:
        return 0.00001
...
metrics_df.to_csv("JKM_Weekly_XGB_3000_LR001_v3.csv", index=False)
...
metrics_df.to_csv("JKM_Weekly_XGB_3000_LR001_v3_MAPE.csv", index=False)
...
model.save_model("JKM_Weekly_model_v3.json")

from xgboost import XGBRegressor

loaded_model = XGBRegressor()
loaded_model.load_model("JKM_Weekly_model_v3.json")
...
X_test.to_csv('JKM_Weekly_X_test.csv', index=True)

y_test.to_csv('JKM_Weekly_y_test.csv', index=True, header=True)

y_pred_df = pd.DataFrame(y_pred, index=X_test.index, columns=['JKM_Price'])

y_pred_df.to_csv('JKM_Weekly_y_pred.csv', index=True, header=True)